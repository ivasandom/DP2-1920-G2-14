...
class FeedersHealthCardNumberPerformanceTests extends Simulation {
	...
	object NewClient1 {
		...
		.check(css("input[name=_csrf]", "value").saveAs("stoken"))
		 ...
		 .doIf("${stoken.exists()}") {
			...
			.formParam("document", "${document}")
			.formParam("documentType", "${documentType}")
			.formParam("healthInsurance", "${healthInsurance}")
			.formParam("healthCardNumber", "${healthCardNumber}")
			.formParam("email", "${email}")
			.formParam("user.username", "${userUsername}")
